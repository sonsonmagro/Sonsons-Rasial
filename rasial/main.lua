...
-- Author: Sonson
-- Description: Start at War's Retreat with desired preset loaded last.
-- Version: 2.0.0
-- Category: PVM
...
------------------------------------------
--[[
    Version 2.0.0 - GUI Update (2026-02-02)
    ---------------------------------------
    Major GUI Overhaul - Inspired by Mochi and Ernie

    NEW FEATURES:
    - Added ImGui-based configuration GUI with tabbed interface
    - Config Tab: Configure rotation preset, bank PIN, health/prayer thresholds
    - Info Tab: Real-time boss health, location, status, and active buffs
    - Metrics Tab: Kill count, GP/hr, fastest/slowest/average kill times
    - Warnings Tab: Display configuration warnings with dismiss button

    - Added JSON config persistence (saves to configs/rasial.config.json)
    - Added pre-start configuration loop (configure before script runs)
    - Added color-coded state display (phases, locations, looting)
    - Added progress bars for boss health and session metrics
    - Added rotation preset dropdown (dynamically loads from rotations.lua)

    CHANGES:
    - Replaced API.DrawTable() with ImGui-based RasialGUI.draw()
    - Refactored tracking() to buildGUIData() pattern

    FILES MODIFIED:
    - rasial/main.lua (this file)
    - rasial/config.lua (simplified defaults, GUI override support)

    FILES CREATED:
    - rasial/gui.lua (new ImGui GUI module)

    CREDITS:
    - GUI patterns inspired by: Mochi and Ernie
--]]
...
-- Apply GUI config to Config module
local guiConfig = GUI.getConfig()
Config.applyGUIConfig(guiConfig)

-- Validate preset items and add warnings for missing items
local function validatePresetItems()
    local preset = Config.userInput.preset
    ...
    if preset.inventory then
        for _, item in ipairs(preset.inventory) do
            ...
            for _, invItem in ipairs(invItems) do
                if invItem.itemid1 == item.id then
                    found = true
                    break
                end
            end
            if not found then
                local itemName = ("Item ID: " .. item.id)
                GUI.addWarning("Missing inventory item: " .. itemName)
            end
        end
    end
    ...
    if preset.equipment then
        ...
        for _, item in ipairs(preset.equipment) do
            local found = false
            if equippedItems and item.slot and equippedItems[item.slot + 1] then
                if equippedItems[item.slot + 1].item_id == item.id then
                    found = true
                end
            end
            if not found then
                local itemName = ("Item ID: " .. item.id)
                RasialGUI.addWarning("Missing equipment: " .. itemName .. " (slot " .. item.slot .. ")")
            end
        end
    end
    ]]

    -- Auto-select warnings tab if there are warnings
...
local Debugging = {
    main = guiConfig.debugMain,
    timer = guiConfig.debugTimer,
    rotationManager = guiConfig.debugRotation,
    playerManager = guiConfig.debugPlayer,
    prayerFlicker = guiConfig.debugPrayer,
    warsRetreat = false
}

API.SetDrawLogs(Debugging.main)
API.printlua("Starting Rasial with preset: " .. guiConfig.rotationPreset, 0, false)
ClearRender()
...
local LOOT            = {
    ...
    UNIQUES_DATA = {
        ...
        [55482] = {
            ...
            thumbnail =
            "https://runescape.wiki/images/thumb/Soulbound_lantern_detail.png/49px-Soulbound_lantern_detail.png?c7346"
        },
        [55488] = {
            name = "Crown of the First Necromancer",
            thumbnail =
            "https://runescape.wiki/images/thumb/Crown_of_the_First_Necromancer_detail.png/78px-Crown_of_the_First_Necromancer_detail.png?c607d"
        },
        [55490] = {
            name = "Robe top of the First Necromancer",
            thumbnail =
            "https://runescape.wiki/images/thumb/Robe_top_of_the_First_Necromancer_detail.png/128px-Robe_top_of_the_First_Necromancer_detail.png?25297"
        },
        [55492] = {
            name = "Robe bottom of the First Necromancer",
            thumbnail =
            "https://runescape.wiki/images/thumb/Robe_bottom_of_the_First_Necromancer_detail.png/76px-Robe_bottom_of_the_First_Necromancer_detail.png?5d7b4",
        },
        [55494] = {
            name = "Hand wrap of the First Necromancer",
            thumbnail =
            "https://runescape.wiki/images/thumb/Hand_wrap_of_the_First_Necromancer_detail.png/88px-Hand_wrap_of_the_First_Necromancer_detail.png?a0465",
        },
        [55496] = {
            name = "Foot wraps of the First Necromancer",
            prefix = "a pair of ",
            thumbnail =
            "https://runescape.wiki/images/thumb/Foot_wraps_of_the_First_Necromancer_detail.png/152px-Foot_wraps_of_the_First_Necromancer_detail.png?6b9d4"
        },
        [55674] = {
            name = "Miso's collar",
            thumbnail =
            "https://runescape.wiki/images/thumb/Miso%27s_collar_detail.png/102px-Miso%27s_collar_detail.png?4e63b"
        }
    }
}
...
local prayerFlicker   = PrayerFlicker.new(Config.userInput.prayerFlicker)

--  Initializing a Player Manager object
local playerManager   = PlayerManager.new({
    health = Config.userInput.playerManager.health,
    prayer = Config.userInput.playerManager.prayer
})
...
local warsRetreat     = WarsRetreat:init({
    ...
    userSettings = {
        bankPin = Config.userInput.bankPin,
        waitForFullHp = Config.userInput.waitForFullHp,
        summonConjures = Config.userInput.warsRetreat and Config.userInput.warsRetreat.summonConjures or true,
        useAdrenCrystal = Config.userInput.warsRetreat and Config.userInput.warsRetreat.useAdrenCrystal or true,
        surgeDiveChance = Config.userInput.warsRetreat and Config.userInput.warsRetreat.surgeDiveChance or 100,
        preset = {
            inventory = Config.userInput.preset.inventory,
            equipment = Config.userInput.preset.equipment
        }
    }
})
...
RasialLobby.tasks     = {
    -- Task: Summon conjures if you can...
    {
        ...
        useTicks = true,
        condition = function()
            return RasialLobby:atLocation()
                and Utils:canUseAbility("Conjure Undead Army")
                and Utils:canUseAbility("Life Transfer")
                and Player:getAnimation() ~= 35502 -- Conjure Undead Army animation
        end,
        action = function()
    ...
    },
    {
        ...
        useTicks = true,
        condition = function()
            return RasialLobby:atLocation() and Utils:canUseAbility("Life Transfer")
        end,
        action = function()
            return Utils:useAbility("Life Transfer")
        end,
        delay = 2,
        delayTicks = true
    },
    {
        ...
        action = function()
            if RasialLobby.variables.instanceStartTime == 0
                or ((os.clock() - RasialLobby.variables.instanceStartTime) > (58 * 60)) -- At least 58 minutes since last instance was created
...
for _, task in pairs(RasialLobby.tasks) do
    timer:addTask(task)
end
...
RasialFight.tasks = {
    {
        ...
        useTicks = true,
        condition = function()
            return RasialFight:atLocation()
                and RasialFight.variables.gateTile == nil
                and not RasialFight.variables.bossDead
        end,
        action = function()
            return RasialFight:gatherData()
        end
    },
    {
        name = "Handle fight",
        ...
        action = function()
            return RasialFight:handleFight()
        end
    },
    {
    ...
    },
    {
        ...
        condition = function()
            return RasialFight:atLocation()
                ...
                and not RasialFight.variables.gateTile ~= nil
                and not RasialFight:isPlayerAtSafespot()
                and not RasialFight.variables.phases[2]
        end,
        action = function()
    ...
    },
    {
        ...
        condition = function()
            return RasialFight:atLocation()
                and RasialFight.variables.gateTile ~= nil
                and RasialFight:getBossInfo().health > 0
                and #RasialFight:getLightning(10) > 0
                and RasialFight.variables.phases[2]
        end,
        action = function()
            return RasialFight:dodgeLightning()
        end,
        parallel = true,
    },
    {
        ...
        condition = function()
            return RasialFight:atLocation()
                ...
                and (RasialFight:getBossInfo().health <= 0)
                and not RasialFight.variables.bossDead
                and not RasialFight:isPlayerAtLootTile()
                and RasialFight.variables.phases[2]
        end,
        action = function()
            if not Player:isMoving() then
    ...
    },
    {
        ...
        condition = function()
            return RasialFight:atLocation()
                and (RasialFight:getBossInfo().health <= 0 or not (RasialFight:getBossInfo().found))
                and RasialFight.variables.gateTile ~= nil
                and RasialFight.variables.phases[2]
                and not RasialFight.variables.bossDead
        end,
        action = function()
            return RasialFight:logKillDetails()
        end
    },
    {
        ...
        useTicks = true,
        condition = function()
            return RasialFight:floorContainsRareLoot()
        end,
        action = function()
            return RasialFight:logUniqueDrop()
        end,
        parallel = true
    },
    {
        ...
        condition = function()
            return RasialFight:atLocation()
                and RasialFight.variables.gateTile ~= nil
                and RasialFight:getBossInfo().health <= 0
                and RasialFight:floorContainsLoot()
                and RasialFight.variables.phases[2]
                and RasialFight.variables.bossDead
        end,
        action = function()
            return RasialFight:pickUpLoot()
        end,
    },
    {
        ...
        condition = function()
            return RasialFight:atLocation()
                and RasialFight.variables.gateTile ~= nil
                and RasialFight:getBossInfo().health <= 0
                and not RasialFight:floorContainsLoot()
                and RasialFight.variables.bossDead
        end,
        action = function()
            return RasialFight:teleportToWarsRetreat()
        end,
        delay = 2,
        delayTicks = true
    }
...
function RasialFight:atLocation()
    return self.constants.location.detector()
end
...
function RasialFight:gatherData()
    ...
    local gate = Utils:findAll(
        self.constants.objects.chamberDoorway.id,
        self.constants.objects.chamberDoorway.type,
        50
    )
    ...
    if #gate > 0 then
        ...
        if self.variables.safeSpot then
            Utils:log(("++ Safe spot tile: (%s, %s, 0)"):format(
                self.variables.safeSpot.x,
                self.variables.safeSpot.y))
        else
        ...
        Utils:log(("++ Gate tile saved in memory: (%s, %s, 0)"):format(
            self.variables.gateTile.x,
            self.variables.gateTile.y))
        return true
    end
    return false
end
...
function RasialFight:getBossInfo()
    local rasial = Utils:findAll(
        self.constants.objects.rasial.id,
        self.constants.objects.rasial.type,
        50
    )
    return {
        found = #rasial > 0,
...
function RasialFight:isPlayerAtSafespot()
    if not self.variables.gateTile or not self.variables.safeSpot then return false end
...
function RasialFight:getLightning(range)
    return Utils:findAll(
        self.constants.objects.lightning.id,
        self.constants.objects.lightning.type,
        range
    )
end
...
function RasialFight:dodgeLightning()
    ...
    if onLightning then
        ...
        local tiles = {}
        local startX, endX = self.variables.finalZone[1].x, self.variables.finalZone[2].x
        local startY, endY = self.variables.finalZone[1].y, self.variables.finalZone[2].y
...
function RasialFight:handlePocketSwitching(bool)
    if bool then
        ...
        if ((self.variables.jasProcCount >= 2
                and jasProc
                and not (API.Container_Get_all(94)[18].item_id == 42787)) or self.variables.phases[2])
            and Inventory:Contains("Erethdor's grimoire")
        then
...
function RasialFight:handleFight()
    if not self.variables.bossDead then
        ...
        if self:atLocation() and not self.variables.phases[1] then
            ...
            rotationManager:load(Config.userInput.preset.rotations.fightRotation)
        end

        -- Phase 2 initialization
        if (self:getBossInfo().health <= 199000) and self:getBossInfo().found
            and not self.variables.phases[2]
        then
            Utils:log("----- FINAL PHASE INITIATED -----")
            self.variables.phases[2] = true
            Utils:log("- Loading final rotation.")
            rotationManager:load(Config.userInput.preset.rotations.finalRotation)
        end
    end

    -- Combat management
    if (self.variables.phases[1] and not self.variables.phases[2]) or (self.variables.phases[2] and self:getBossInfo().health > 0) then
        playerManager:setBuffConfig(Config.userInput.preset.buffs)
        prayerFlicker:update()
        rotationManager:execute()
    else
...
function RasialFight:logUniqueDrop()
    ...
    if uniqueDrops then
        ...
        for _, drop in pairs(uniqueDrops) do
            local killData = Common.variables.killData[Common.variables.killCount]
            local dropData = LOOT.UNIQUES_DATA[drop.Id]
            table.insert(Common.variables.uniquesLooted,
                ({ "-- [" .. Common.variables.killCount .. "] " .. dropData.name, killData.runtime }))

            -- Send Discord notification
            if Config.userInput.useDiscord then
                --- @diagnostic disable-next-line
                local embed = DiscordEmbed.new()
                    :SetTitle(string.format("Congratulations! You found %s%s",
                        dropData.prefix or (dropData.name ~= "Miso's collar") and "a " or "", dropData.name))
                    :SetDescription("You've managed to strip Rasial of a shiny **" .. dropData.name .. "**!")
                    :SetColor(10181046)
                    :SetTimestamp(tostring(os.time()))
                    --- @diagnostic disable-next-line
                    :SetThumbnail(EmbedImage.new(dropData.thumbnail, dropData.thumbnail, 50, 50))
                    --- @diagnostic disable-next-line
                    :SetAuthor(EmbedAuthor.new(string.format("[%s] Sonson's Rasial", Common.variables.scriptVersion), "",
                        "https://runescape.wiki/images/thumb/The_First_Necromancer_chathead.png/90px-The_First_Necromancer_chathead.png?dea03",
                        ""))
                    --- @diagnostic disable-next-line
                    :AddField(EmbedField.new("Kill Count", Common.variables.killCount, true))
                    --- @diagnostic disable-next-line
...
function RasialFight:getLoot()
    return API.GetAllObjArray1(
        Utils:virtualTableConcat(
            LOOT.COMMONS,
            LOOT.UNIQUES
        ), 70, { 3 }
    )
...
function RasialFight:floorContainsLoot()
    return #self:getLoot() > 0
end
...
function RasialFight:pickUpLoot()
    ...
    if API.LootWindowOpen_2() and self:floorContainsLoot() then
        if API.DoAction_LootAll_Button() then
            Common.variables.gp = Common.variables.gp + Utils:getLootWindowAmount()
...
-- Add tasks to timer object
for _, task in pairs(RasialFight.tasks) do
    timer:addTask(task)
end
...
timer:addTask({
    ...
    useTicks = true,
    condition = function()
        return (warsRetreat:atLocation() and RasialFight.variables.gateTile ~= nil)
    end,
    action = function()
...
timer:addTask({
    ...
    parallel = true,
    condition = function()
        return warsRetreat:atLocation() and (API.Container_Get_all(94)[18].item_id == 51814) and
...
local function buildGUIData()
    ...
    local killcountPerHour = Utils:valuePerHour(killcount, Common.variables.scriptStartTime)

    -- GP/hr calculations
    local perHour = Utils:valuePerHour(Common.variables.gp, Common.variables.scriptStartTime)
    local perHourNumber = tonumber(perHour) or 0

    -- Kill data calculations
    local fastestKill, slowestKill, averageKill = Utils:getKillStats(Common.variables.killData)
    ...
    local activeBuffs = {}
    for _, buff in ipairs(Config.userInput.preset.buffs) do
...
local function tracking()
    ...
    local uniquesLootedPerHour = Utils:valuePerHour(uniquesLooted, Common.variables.scriptStartTime)
    ...
    for i, killData in pairs(Common.variables.killData) do
        Utils:tableConcat(formattedKillData,
            { { string.format("- [%s] %s", i, killData.runtime), killData.fightDuration } })
    end
    ...
    local metrics = {
        { string.format("[%s] Sonson's Rasial", Common.variables.scriptVersion), API.ScriptRuntimeString() },
        separator,
        { "Metrics:",                                                            "" },
        { "- Status",                                                            timer:getStatus() },
        { "- Location",                                                          location },
        { "- Total Kills (/hr)",                                                 string.format("%s (%s)", killcount, killcountPerHour) },
        { "- Total Rares (/hr)",                                                 string.format("%s (%s)", uniquesLooted, uniquesLootedPerHour) },
        { "- Total GP (/hr)",                                                    string.format("%s (%s)", currentGp, gpPerHour) },
        separator,
        { "Kill Times:",          "" },
        { "- Fastest Kill:",      fastestKill },
        { "- Slowest Kill:",      slowestKill },
        { "- Average Kill Time:", averageKill },
    }
    ...
    local rasialFightVariables   = Utils:generateTable("Rasial Fight Variables:", RasialFight.variables)
    local rasialLobbyVariables   = Utils:generateTable("Rasial Lobby Variables:", RasialLobby.variables)
    local warsRetreatVariables   = Utils:generateTable("War's Retreat Variables:", warsRetreat.variables)
    local commonVariables        = Utils:generateTable("Common Variables:", Common.variables)
    ...
    local rasialInfo             = {
        ...
        { "- Tile",              tostring(RasialFight:getBossInfo().tile.x .. ", " .. RasialFight:getBossInfo().tile.y) },
